...
void listarProduto() {
    for (const auto &produto : listaDeProdutos) {
        cout << "Produto: " << produto.nomeProduto;
...
void excluirProduto() {
    ...
    cin.ignore();
    getline(cin, nome);
    for (auto it = listaDeProdutos.begin(); it != listaDeProdutos.end(); ++it) {
        if (it->nomeProduto == nome) {
            listaDeProdutos.erase(it);
...
void ordenarProduto() {
    listaDeProdutos.sort(comparaProdutos);
}
